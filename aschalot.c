/* aschalot - generates vanity .anon names using brute-force hashing.
 * A fork of eschalot, which was a fork of shallot. */
	...
	while (!done) {
		...
		while  ((e < RSA_E_LIMIT) && !done) {
			...
			if (search(buf, anon)) {
				...
				if (strncmp((char *)anon, (char *)onionfinal,
				    (!rflag ? strnlen((char *)anon, ONION_LENP1) : 16))) {
					msg("\nWARNING! Error detected! CPU/RAM overheating?\n");
					msg("Found %s, but finalized to %s.\n",
					    (char *)anon, (char *)onionfinal);
					...
					printresult(rsa, anon, onionfinal);
...
/* Print found .anon name and PEM formatted RSA key. */
void 
...
{	
	...
	msg("Found a key for %s (%d) - %s.anon\n",
...
/* Generate .anon name from the RSA key. */
void
onion_enc(uint8_t *anon, RSA *rsa)
{
...
_Bool	
fsearch(uint8_t *buf, uint8_t *anon)
{
	unsigned int i;
	uint16_t ind;
	uint64_t wrd;
...
/* Regex mode search. */
_Bool	
rsearch(__attribute__((unused)) uint8_t *buf, uint8_t *anon)
{
	return !regexec(regex, (char *)anon, 0, 0, 0);
}

/* Fixed prefix mode search. */
_Bool	
psearch(__attribute__((unused)) uint8_t *buf, uint8_t *anon)
{
	return !memcmp(anon, prefix, prefixlen);
}
